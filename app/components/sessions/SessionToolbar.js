...
export default function SessionToolbar(session) {
  ...
  <nav class="toolbar color-contrast" hx-boost="true" hx-push-url="true">
    <ul class="layout-column">
      ...
      <li><a id="nav-link-script" class="${ getActiveClass('documents') }" href="${ session.toURL('?view=documents') }">${Icon("script")}</a></li>
    </ul>
  </nav>
  `;
}