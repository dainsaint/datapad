...
export default function EpisodeToolbar({ episode = new Episode() } = {}) {
  ...
  return html`
    <nav class="toolbar ribbon" hx-boost="true" hx-push-url="true">
      ...
      <a id="nav-link-documents" class="${ getActiveClass('documents') }" href="${ episode.toURL('/documents') }">${Icon("script")}</a>
      <a id="nav-link-settings" class="${ getActiveClass('settings') }" href="${ episode.toURL('/settings') }">${Icon("gears")}</a>
